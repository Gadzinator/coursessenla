...
WITH calculated_costs AS (SELECT CONCAT(m.firstname, ' ', m.surname) AS member,
                                 f.name                              AS facility,
                                 CASE
                                     WHEN m.mem_id = 0 THEN b.slots * f.guest_cost
                                     ELSE b.slots * f.member_cost
                                     END                             AS cost
                          FROM cd.members AS m
                                   INNER JOIN cd.bookings AS b ON m.mem_id = b.mem_id
                                   INNER JOIN cd.facilities AS f ON b.fac_id = f.fac_id
                          WHERE b.start_time >= '2012-07-03'
                            AND b.start_time < '2012-07-04')
