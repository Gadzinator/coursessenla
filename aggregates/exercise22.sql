...
SELECT reporte.date,
       (SELECT SUM(CASE
                       WHEN mem_id = 0 THEN slots * f.guest_cost
                       ELSE slots * member_cost
           END) AS revenue
        FROM cd.bookings b
                 INNER JOIN cd.facilities f
                            ON b.fac_id = f.fac_id
        WHERE b.start_time > reporte.date - INTERVAL '14 days'
          AND b.start_time < reporte.date + INTERVAL '1 day') / 15 AS revenue
FROM (SELECT CAST(GENERATE_SERIES(TIMESTAMP '2012-07-01', '2012-07-31', '1 day') AS DATE) AS date) AS reporte
ORDER BY reporte.date;