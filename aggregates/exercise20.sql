...
FROM (SELECT f.name,
             NTILE(3) OVER (ORDER BY SUM(CASE WHEN b.mem_id = 0 THEN b.slots * f.guest_cost ELSE b.slots * f.member_cost END) DESC) AS revenue_group
      FROM cd.bookings b
               INNER JOIN cd.facilities f ON b.fac_id = f.fac_id
      GROUP BY f.name) AS facility_revenue
